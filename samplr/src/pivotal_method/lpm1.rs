...
struct LocalPivotalMethod1<'a> {
    tree: Tree<'a>,
    idx: Indices,
    candidates: Vec<usize>,
}

pub fn local_pivotal_method_1(
...
impl<'a> DrawVariant for LocalPivotalMethod1<'a> {
    fn len(&self) -> usize {
        self.idx.len()
    }
    ...
    fn draw(&mut self, rand: &RandomGenerator) -> Option<(usize, usize)> {
        ...
        loop {
            ...
            if self.candidates.len() > 0 {
                let id2 = self.candidates.draw(rand);
                return Some((id1, id2));
            }
        }
    }
...
mod tests {
    ...
    fn lpm1_draw() {
        ...
        let mut variant = LocalPivotalMethod1 {
        ...
        assert_eq!(variant.draw(&RAND99), Some((9, 4)));
    }

    /*
    // Can't implement w/o intricate mock of random generator
    fn lpm1() {
        let data = Matrix {
            dt: &[
                0.26550866, 0.37212390, 0.57285336, 0.90820779, 0.20168193, 0.89838968, 0.94467527,
        ...
        let mut lpm = local_pivotal_method_1(
            || 0.0,
